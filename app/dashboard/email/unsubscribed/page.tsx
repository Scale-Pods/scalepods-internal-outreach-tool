...
export default function UnsubscribedPage() {
    ...
    const filteredResults = unsubscribed.filter(u => {
        if (filterType !== "all" && u.leadType !== filterType) return false;
    ...
    const coldCount = unsubscribed.filter(u => u.leadType === 'cold').length;
    const hotCount = unsubscribed.filter(u => u.leadType === 'hot').length;
    ...
    return (
        <div className="space-y-6 pb-10 pt-6 relative min-h-[500px]">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <StatCard title="Total Unsubscribed" value={unsubscribed.length.toString()} />
                <StatCard title="Cold Unsubscribed" value={coldCount.toString()} color="text-blue-600" />
                <StatCard title="Hot Unsubscribed" value={hotCount.toString()} color="text-orange-600" />
            </div>
            ...
            <div className="space-y-3">
                {!loading && paginatedResults.length === 0 ? (
                    ...
                    paginatedResults.map((lead, index) => <UnsubscribeCard key={index} lead={lead} />)
                )}
            </div>
...
function UnsubscribeCard({ lead }: { lead: UnsubscribedEntry }) {
    return (
            ...
            <div className="flex-1 min-w-0 grid grid-cols-1 md:grid-cols-12 gap-4 items-center">
                ...
                <div className="md:col-span-3">
                    <Badge
                        ...
                        className={cn(
                            "font-bold gap-1",
                            lead.leadType === 'cold' ? "bg-blue-50 text-blue-700 border-blue-200" : "bg-orange-50 text-orange-700 border-orange-200"
                        )}
                    >
                        {lead.leadType === 'cold' ? <Snowflake className="h-3 w-3" /> : <Flame className="h-3 w-3" />}
                        {lead.leadType === 'cold' ? 'Cold' : 'Hot'}
                    </Badge>
                </div>
                <div className="md:col-span-4 text-right">
                    <span className="text-xs text-slate-400 font-medium">{formatDate(lead.date)}</span>
                </div>
            </div>
        </div>
    );
