...
export async function GET(req: Request) {
    ...
    const tableName = "TradeIndia_leads";
    let allData: any[] = [];
    let offset = 0;
    const limit = 1000;
    let hasMore = true;
    ...
    try {
        while (hasMore) {
            ...
            // Note: Date filtering could be added here if the table has a date column
...
export async function DELETE(req: Request) {
    ...
    try {
        const response = await fetch(`${baseUrl}/TradeIndia_leads?campaign_name=eq.${encodeURIComponent(campaignName)}`, {
            method: 'DELETE',
            headers
        });
