...
-- which defines _is_truthy/_safe_ts that this depends on).
--
-- Full RPC replacement for lib/services/email-outreach.ts's
-- fetchOutreachLeads() + normalizeRow() — returns the complete
-- NormalizedLeadRow shape (identity, stages[], replies[], flags) as one
-- jsonb row per lead, built entirely in SQL via jsonb_build_object /
-- jsonb_agg. Used by the Sent and Received email pages, which render
-- actual email/reply content (not just counts), so a plain aggregate RPC
    ...
    master_cold as (
        select
            jsonb_build_object(
                ...
                -- master_cold_leads has BOTH content (User_Replied_N/Bot_Replied_N)
                -- and separate _Status_N JSON-metadata columns; a reply counts if
                -- either the content or its status flag is truthy, matching
                -- normalizeRow()'s hasUser/hasBot logic exactly.
                'replies', (
                    select coalesce(jsonb_agg(jsonb_build_object(
                        'index', idx,
                        'userReplied', case when _is_truthy(uc) then uc else null end,
