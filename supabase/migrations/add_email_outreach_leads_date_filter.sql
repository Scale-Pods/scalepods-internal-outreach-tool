-- Run this after add_email_outreach_leads_pagination_fix.sql.
--
-- Adds server-side date-range filtering to get_outreach_leads — the Sent
-- and Received email pages were fetching the ENTIRE lead universe (4500+
-- rows, full HTML email bodies, all 25 reply slots each = ~9.4MB payload)
-- on every load and filtering client-side, which is the real source of the
-- 4-11s load time reported for "last 7 days" view. Filtering by
-- lastContacted/createdAt in SQL means only leads actually in range are
-- ever serialized and shipped over the wire.
--
-- Return shape is unchanged, so this stays a plain CREATE OR REPLACE (new
-- optional trailing params, existing callers omitting them still work).
...
as $$
    with enriched as (
        select
            jsonb_build_object(
                ...
                'effectiveDate', coalesce("Email Last Contacted", created_at)
            ) as lead_json,
    ...
    master_cold as (
        select
            jsonb_build_object(
                ...
                'effectiveDate', coalesce("Email Last Contacted", email_last_sent_at, created_at)
            ) as lead_json,
    ...
    all_leads as (
        select lead_json, effective_date from enriched
        union all
        select lead_json, effective_date from master_cold
        union all
        select lead_json, effective_date from hubspot
    )
    select lead_json
    from all_leads
